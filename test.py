...
# Split the data into 2 folds for cross-validation
    num_folds = 2
    fold_size = len(file_path_label_pairs) // num_folds

    # Create a list of data loaders for each fold
    train_loaders = []
    val_loaders = []

    for i in range(num_folds):
        # Select the data for this fold
        val_data = file_path_label_pairs[i*fold_size:(i+1)*fold_size]
        train_data = file_path_label_pairs[:i*fold_size] + file_path_label_pairs[(i+1)*fold_size:]
        ...
        # Add the data loaders to the lists for this fold
        train_loaders.append(train_loader)
        val_loaders.append(val_loader)

    return train_loaders, val_loaders

def k_fold(trainer, model):
    val_loss, models = [], []
    train_loaders, val_loaders = k_fold_data()
    for i, (train_loader, val_loader) in enumerate(zip(train_loaders, val_loaders)):
        print(f"Training on fold {i + 1}")
        trainer.fit(model, train_loader, val_loader)
        val_loss_epoch = trainer.evaluate_loss(val_loader)
